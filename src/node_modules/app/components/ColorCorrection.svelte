...
<script>
	...
	export let options = $colorCorrection;

	const getColorCorrection = opts => !opts.active
		? ''
		: [
			includeIfNot(0, opts, 'invert', '%'),
			includeIfNot(0, opts, 'hue-rotate', 'deg'),
			includeIfNot(0, opts, 'grayscale', '%'),
			modelFormatters[opts.colorModel],
	...
	const getCvdSimulation = opts => !opts.active
		? ''
		: simulationFormatters[opts.simulate];
