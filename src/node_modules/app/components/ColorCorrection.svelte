...
<script>
	import {colorCorrection, cvdSimulation} from 'app/stores/color';
	...
	const getColorCorrection = opts => !opts.active
		? ''
		: [
	...
	const getCvdSimulation = opts => !opts.active
		? ''
		: simulationFormatters[opts.simulate](opts);

	$: $colorCorrection = ['--color-correction', getColorCorrection(options)];
	$: $cvdSimulation = ['filter', getCvdSimulation(options)];
</script>
