...
import Accessibility from '../../../routes/accessibility.svelte';
	...
	$: showLabels = Math.max(...($a11ySettings[currentId].values?.map(i => i.toString().length) || [0])) < 5;
	$: label = `${$a11ySettings[currentId].label}: ${formatValue($a11ySettings[currentId].value)}`;
</script>
