<script>
	...
	import { isNot, makeMergeAppliedFnMap } from '@svizzle/utils';
	...
	$: disableReset = isGroupReset($a11ySettings);
...
<dialog class={screen?.classes}>
	<nav class='resets'>
		<div class='text' on:click={() => resetGroup('text')}>
			<Icon
				fill={disableReset.text ? 'silver' : 'black'}
				glyph={FormatClear}
				stroke={'none'}
			/>
		</div>
		<div class='color' on:click={() => resetGroup('color')}>
			<Icon
				fill={disableReset.color ? 'silver' : 'black'}
				glyph={ColorClear}
				stroke={'none'}
			/>
		</div>
