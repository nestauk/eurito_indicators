...
<script>
	...
	const modelCycler = makeCycleUsingAndStoreProp(
		colorModels,
	...
	*/
	...
	const toggleActive = () => 
		$colorCorrectionOptions.active = !$colorCorrectionOptions.active;
	const toggleInvert = () => 
		$colorCorrectionOptions.invert = $colorCorrectionOptions.invert === 100
			? 0
			: 100;
...
<section>
	...
	<menu>
		{#if $selectedProperty === 'typeface'}
			<label for=''>Font: {$fontFamily}</label>
			<div class='layout-1'>
				{#each fonts as font}
		...
		{:else if $selectedProperty === 'fontScaling'}
			<label for=''>Font scale: {Math.round($fontScaling*100)}%</label>
			<div class='layout-2'>
				{#each fontScales as scale}
					<div class='.layout-2'>
						<label for={'scale-'+scale}>{Math.round(scale * 100)}%</label>
						<input name={'scale-'+scale} type="radio" bind:group={$fontScaling} value={scale}>
					</div>
				{/each}
			</div>
		{:else if $selectedProperty === 'lineHeight'}
			<label for=''>Line height: {Math.round($lineHeight*100)}%</label>
			<div class='layout-2'>
				{#each lineHeights as height}
					<div class='.layout-2'>
						<label for={'height-'+height}>{Math.round(height * 100)}%</label>
						<input name={'height-'+height} type="radio" bind:group={$lineHeight} value={height}>
					</div>
				{/each}
			</div>
		{:else if $selectedProperty === 'letterSpacing'}
			<label for=''>Letter spacing: {Math.round($letterSpacing*100)}%</label>
			<div class='layout-2'>
				{#each letterSpacings as spacing}
					<div class='.layout-2'>
						<label for={'letterspacin-'+spacing}>{Math.round(spacing * 100)}%</label>
						<input name={'letterspacin-'+spacing} type="radio" bind:group={$letterSpacing} value={spacing}>
					</div>
				{/each}
			</div>
		{:else if $selectedProperty === 'wordSpacing'}
			<label for=''>Word spacing: {Math.round($wordSpacing*100)}%</label>
			<div class='layout-2'>
				{#each wordSpacings as spacing}
					<div class='.layout-2'>
						<label for={'wordspacing-'+spacing}>{Math.round(spacing * 100)}%</label>
						<input name={'wordspacing-'+spacing} type="radio" bind:group={$wordSpacing} value={spacing}>
					</div>
				{/each}
			</div>
		{:else if $selectedProperty === 'invert'}
			<label for='invert'>Invert</label>
			<Switch
				value={$colorCorrectionOptions.invert === 0 ? 'No' : 'Yes'}
				values={['No', 'Yes']}
				on:toggled={event => {
					$colorCorrectionOptions.invert = event.detail === 'No' ? 0 : 100
				}}
			/>
		{:else if $selectedProperty === 'hue'}
			<label for='hue'>Hue shift: {$colorCorrectionOptions['hue-rotate']}%</label>
			<div class='.layout-2'>
				<input name='hue' type="range" min='0' max='360' bind:value={$colorCorrectionOptions['hue-rotate']}>
			</div>
		{:else if $selectedProperty === 'colorModel'}
			<label for='' >Color deficiency: {$colorCorrectionOptions.colorModel}</label>
			<div class='.layout-1'>
				{#each colorModels as model}
					<input name={'model-'+model} type="radio" bind:group={$colorCorrectionOptions.colorModel} value={model}>
				{/each}
			</div>
		{:else if $selectedProperty === 'grayscale'}
			<label for='grayscale'>Grayscale: {$colorCorrectionOptions.grayscale}%</label>
			<div class='.layout-2'>
				<input name='grayscale' type="range" min='0' max='100' bind:value={$colorCorrectionOptions.grayscale}>
			</div>
		{:else if $selectedProperty === 'brightness'}
			<label for='brightness'>Brightness: {$colorCorrectionOptions.brightness}%</label>
			<div class='.layout-2'>
				<input name='brightness' type="range" min='0' max='100' bind:value={$colorCorrectionOptions.brightness}>
			</div>
		{:else if $selectedProperty === 'contrast'}
			<label for='contrast'>Contrast: {$colorCorrectionOptions.contrast}%</label>
			<div class='.layout-2'>
				<input name='contrast' type="range" min='0' max='100' bind:value={$colorCorrectionOptions.contrast}>
			</div>
		{/if}
	</menu>
	<nav>
