...
The selected NUTS2 regions can be highlighted or filtered using the switch in
the first section:
...
map of the UK for the NUTS2 regions in the selected year.
...
&quot;TRAC peer group A&quot;). This group has been chosen due to its key
relevance for BEIS regional policies.

#### Bar chart

The bar chart in this view shows the indicator values for each NUTS2 region in
descending order.
...
That zip file also contains an extra CSV file (titled <code>{allNUTS2IndicatorsCsvName}</code> 
