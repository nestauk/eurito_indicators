...
def make_indicator(
    ...
    for cat, table in split_tables.items():
        ...
        with open(
            ...
            json.dump(sch, outfile, indent=0)
...
def make_schema(indicator_type, category, suffix="count"):
    ...
    if indicator_type == "article_sources":
        ...
        schema[
            "title"
        ] = f"Number of participations in {AI_CLUSTERS[category]} preprints"
        schema[
            "subtitle"
        ...
        schema[
            "title"
        ] = f"Relative specialisation {AI_CLUSTERS[category]} preprints"
        schema[
            "subtitle"
        ...
        schema["schema"]["value"][
            "label"
        ] = f"Relative specialisation in {AI_CLUSTERS[category]} preprints"
        schema["schema"]["value"]["id"] = f"{category}_{suffix}"
