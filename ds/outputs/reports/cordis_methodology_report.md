...
One potential explanation for the overrepresentation of a topic in the preprints corpus is that it benefits from / requires / involves techniques that allow rapid dissemination of results. 
...
In subsequent analyses of the preprints corpus we use the `topSBM` model mentioned previously to cluster papers into different categories which we then use to analyse temporal trends and geographical differences. We take several steps to improve the interpretability of these clusters and to assess their robustness.

1. We name each cluster using the most salient (highest TF-IDF) words in the documents it contains
2. We build an algorithm that fits multiple KMeans clustering models on a random sample of the data data (i.e. papers are clustered based on the similarities in the topic distribution generated by the `topSBM` model). We then:
    * Estimate the average frequency with which pairs of documents in different `topSBM` clusters are assigned to the same / different clusters during the KMeans check.
    * Estimate the average correlation between the distribution of assignments to Kmeans clusters for documents in different `topSBM` clusters.

This analysis is particularly useful for identifying unstable clusters with observations that are not consistently paired together by an alternative clustering approach, and identifying which sets of clusters generated by `topSBM` have a higher degree of overlap.

As the correlation plot below shows, in broad terms documents classified in a `topSBM` cluster also tend to be placed together in our alternative clustering scheme (more activity in the diagonal). Having said this, we detect some spillage between clusters but this is generally interpretable: for example, there are overlaps between `cluster2`, `cluster 10` and `cluster 20`, all of which focus on pandemic related policies, social distancing and lockdowns. `cluster 3` and `cluster 4`, related to genomics, also display a high degree of overlap.

![Average co-occurrences between documents in topSBM clusters](val_figures/png/cluster_cooccurrences.png)

When we consider the distribution of correlations in the test allocations of various clusters in the boxplot below, this shows that clusters related to policy, healthcare and social aspects and impacts of the pandemic tend to have a stronger correlation with other clusters, consistent with the idea of a degree of overlap and fuzziness in the boundaries between policy-oriented research areas, while technical subjects -towards the bottom - tend to be easier to cluster into homogeneous groups.
