...
We begin with an analysis of the European Commission funded response to Covid-19 under the rubric of the Horizon 2020 Framework Programme using the CORDIS open grant dataset including information about H2020 funded projects and the organisations participating in them.^[We recognise that this does not capture all EC contributions to the R&I fight against the pandemic, for example through the activities of the European Research Council and EC contributions to the [Innovative Medicines Initiative](https://www.imi.europa.eu/apply-funding/closed-calls/imi2-call-21).] There were three areas of interest in this analysis: 
...
We use the Specter language model to create vector representations of H2020 projects that were not included in our Covid-19 labelled dataset. This allows us to identify projects which are semantically similar (close in vector space) to Covid-19 research. We will use the resulting information in two ways:
...
2. Characterise existing research capabilities in research areas related to the pandemic in different countries _before it happened_ which might explain their subsequent levels of involvement in the coordination of Covid-19 projects.
...
[@fig:dist_distr] presents the distributions of distances to those signatures projects inside a Covid-research cluster (in orange) and projects outside (in blue), 
...
The figure shows, unsurprisignly, that projects that are part of a Covid-19 cluster tend to be closer to its centroid than those outside. 

We also see notable differences between Covid-19 research clusters. In the case of cluster 0 (Diagnosis and Prevention) and cluster 5 (Biotechnology) there is a high degree of separation between Covid-19 and non-Covid-19 clusters while cluster 1, 3 and 4 (public health, policy and products and services) there is strong overlap between Covid and non-Covid 19 projects suggesting fuzzier semantic boundaries between both categories and the risk of many false positives if we do not adjust a threshold to define relatedness to a Covid-19 research cluster carefully. In order to do this, we will draw on a measure of dispersion in the distribution of distances to each of the Covid-19 clusters: any non-Covid project that is two standard deviations closer to the centroid of a Covid-19 research cluster than the mean is defined as "semantically similar" to that cluster. As the green vertical lines in {#fig:dist_distr} this thresold is more demanding for fuzzier Covid-19 research clusters.

In total, we identify 6,244 projects "close" to various Covid-19 research clusters. We present some (randomly drawn) examples in table [@#tbl:prox_ex]. We see that projects close to health and biomedical research clusters are highly related or adjacent to Covid-19 use cases (e.g. use of AI methods to analyse CT scans, development of testing devices, RNA analysis in plant infections) while the relevance of policy, networking and product and services projects is somewhat looser (tackling energy poverty in urban settings, development of lightweight materials).  
...
: Examples of research projects that are semantically close to Covid-19 research clusters {#tbl:prox_ex}

[@fig:related_trend] presents the volumes of activity in Covid-19 related projects (excluding projects in the initial labelled set). It shows relative stability in the number of projects in Covid-19 / societal emergency areas over time, although we note some spikes of activity which might be related to specific funding calls in 2019 and 2021 that might be interesting to analyse further.

![Evolution of activity in Covid-19 related research areas](final_report_deck/png/relatedness_trends.png){#fig:related_trend}

### Measuring national capabilities in Covid-19 related areas - before Covid-19.

We use the measures of relatedness to identify corpora of projects related to various Covid-19 research clusters _before the pandemic_. We then calculate relative specialisation of countries in each of those research clusters based on the number of projects that they have coordinated within them, which we consider a proxy for the presence of established capabilities and networks in those research areas. Are countries with such capabilities more active when the pandemic arrives?

One issue here is that those measures of specialisation might be contingent on the Covid-19 relatedness thresholds that we have selected. We assess their robustness by estimating the levels of specialisation for different thresholds (between 1.5 and 2.5 standard deviations closer to the centroid of the covid cluster than the mean for non-Covid projects) and present the results in [#fig:sim_interv]. 

![Pre-Covid-19 intervals for specialisation in Covid-19 related areas based on different similarity thresholds. The vertical axis is sorted by the mean of the midpoint in the confidence interval for all research clusters for a country](final_report_deck/png/specialisation_robustness.png){#fig:sim_interv}

The chart suggests that the measures of specialisation in Covid research clusters such as Products and Services and Policy - where as we noted below there is a higher degree of semantic overlap between Covid and non-Covid projects are particularly volatile. In contrast, estimates of specialisation in biomedical research clusters such as Diagnosis and Prevention and Biotechnology are more robust.

In [@prep_plot] we compare specialisation in Covid-19 related topics before (horizontal axis) and after the pandemic for the top 15 countries in the data, with Covid research clusters sorted by the strength of the rank-correlation between specialisation before the pandemic and specialisation afterwards.
...
The chart shows a strong correlation between pre and post specialisation in some research areas such as Biotechnology, Policy and Public Health while the correlation is negative in the Products and Services research cluster (where, as we just pointed out, our measures of specialisation pre-pandemic are noisier). Understanding what drives the differences across cluster research areas as well as what explains whether countries exploited - or not - their research strenghts in Covid-19 related areas when the pandemic arrived important questions for future research.
